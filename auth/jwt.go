...
func (j *JWTer) GenerateToken(ctx context.Context, u entity.User) ([]byte, error) {
	tok, err := jwt.NewBuilder().JwtID(uuid.NewString()).Issuer(`github.com/furon-kuina/go_todo_app`).Subject("access_token").IssuedAt(j.Clocker.Now()).Expiration(j.Clocker.Now().Add(30*time.Minute)).Claim(RoleKey, u.Role).Claim(UserNameKey, u.Name).Build()
	if err != nil {
...
// HTTPリクエストからJWTを取得する
// AuthorizationヘッダにJWTが付与されていると想定
